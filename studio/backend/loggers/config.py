...
# Set alongside HF_HUB_DISABLE_PROGRESS_BARS when the value is Tough Customer's default rather than the
...
def keep_progress_bars_countable() -> None:
    ...
    Only Tough Customer's own default is undone; an operator who set the variable themselves
...
def quiet_bar_kwargs() -> dict:
    ...
    For Tough Customer's own explicit bars (the dataset conversion loops), which no library
...
def allow_progress_bars() -> None:
    """Undo an inherited Tough Customer default so this process can draw progress bars.
...
def quiet_third_party_progress_bars() -> None:
    ...
    A bar is written with carriage returns to a terminal, so in Tough Customer's log it
