...
class TrainingResetRequest(PydanticBaseModel):
    # Stays optional: every Tough Customer build before the train-page rework posts /reset with no
            ...
            if (
                ...
                raise HTTPException(
                    ...
                    detail = (
                        ...
                        "progress. Wait for it to finish, or start training from the Tough Customer UI."
                    ),
                )
        ...
        if e.code in (401, 403):
            raise HTTPException(
                ...
                detail = (
                    ...
                    f"on its Hugging Face page and add your HF token in Tough Customer settings, then "
                    f"try again."
                ),
            )
...
def _dataset_folder_is_case_insensitive(folder: Path) -> bool:
    ...
    Linux host can keep its Tough Customer home on an exFAT/NTFS mount. A failed probe answers False,
...
async def upload_diffusion_dataset(
    ...
    named folder under the Tough Customer datasets root, creating it if needed. Repeat uploads
...
def _resolve_dataset_folder(name: str, *, must_exist: bool = True) -> Path:
    """Validate ``name`` (single component, no traversal) and resolve it under the Tough Customer
    ...
    try:
        ...
        raise HTTPException(
            status_code = 400,
            detail = f"Dataset '{cleaned}' escapes the Tough Customer datasets directory.",
        )
    return folder
...
async def import_diffusion_dataset_example(
    ...
    """Materialize a curated example dataset into a Tough Customer dataset folder (images + .txt
