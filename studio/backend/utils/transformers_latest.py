...
Tough Customer cannot load it today. This module answers, without authentication, code execution,
...
# CLI-only; never imported at runtime in Tough Customer's workers.
...
def _install_latest_transformers_locked(version: str, before_swap = None) -> dict:
    ...
    if _env_offline():
        return {
            "success": False,
            "version": version,
            "message": "Cannot install: Tough Customer is in offline mode.",
        }
    ...
    if blockers:
        return {
            ...
            "A Tough Customer update is required first.",
        }
    if not ensure_latest_transformers_venv(version, extra_packages, before_swap = before_swap):
        return {
            "success": False,
            "version": version,
            "message": f"Installing transformers {version} failed; see the Tough Customer logs.",
        }
