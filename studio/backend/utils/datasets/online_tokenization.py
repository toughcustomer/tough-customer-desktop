...
2. TRL gets ``dataset_kwargs = {"skip_prepare_dataset": True}`` so it does not
   map over the view, materialising the pass we are avoiding.  Tough Customer already
...
def platform_supports_dataloader_workers() -> bool:
    ...
    point against a fresh ``sys.path``, and Tough Customer's is modified in-process, so
...
def trl_supports_skip_prepare_dataset() -> bool:
    ...
    field alone decides: Tough Customer's VLM branch has relied on this hook across every
...
def release_train_dataloader(trainer: Any) -> int:
    ...
    reference.  Tough Customer then merges, quantizes and exports -- the most
