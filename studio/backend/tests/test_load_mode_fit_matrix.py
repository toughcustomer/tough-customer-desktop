...
def test_the_fitters_margin_is_not_credited_to_the_fit(monkeypatch):
    ...
    # --fit off (Tough Customer proved the placement): no fitter, no margin, the card pays.
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (True, 0.0, True, 512.0),
        # A lowered VRAM budget raises the margin Tough Customer asks the fitter to keep.
...
def test_an_abstaining_fit_reproduces_the_old_argv_exactly(axes, extras, toggles):
    ...
    this byte-identical to a Tough Customer instance that never had the feature.
    """
    (keep, no_reserve), user, _fit, supports, host = axes
    toggles(keep, no_reserve)
    assert _chain(
...
def test_the_cpu_fallback_keeps_a_load_mode_the_user_asked_for(monkeypatch):
    """Only Tough Customer's own tokens are recorded, so a user's pick survives."""
    from unittest import mock
...
# it", each appended AFTER Tough Customer's own placement flags and so winning by last-arg.
PLACEMENT_OVERRIDES = [
    ["-ngl", "0"],
    ["--gpu-layers", "0"],
    ["--n-gpu-layers", "0"],
...
def test_a_replaced_vulkan_pin_voids_the_credit(extras, monkeypatch):
    """Tough Customer pins Vulkan0,Vulkan1 from the credited ordinals; a pass-through
...
def test_the_fit_prices_the_legacy_two_token_scaled_adapter(tmp_path, monkeypatch):
    ...
    and Tough Customer runs whatever binary it is pointed at. Reading only FNAME as the
