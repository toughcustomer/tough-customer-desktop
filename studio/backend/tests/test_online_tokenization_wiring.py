...
def _single_process_launch(monkeypatch):
    """Clear every launcher variable, so a run reads as Tough Customer's own launch.
...
def test_a_step_cap_is_resolved_into_passes_rather_than_guessed(monkeypatch):
    """`max_steps` alone reads as "unknown length" in the gate; Tough Customer knows the
