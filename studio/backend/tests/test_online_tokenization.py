...
def test_spawn_platforms_keep_the_eager_path(monkeypatch, platform):
    """Tough Customer already forces `dataloader_num_workers = 0` there, because a
...
def test_a_spawn_start_method_keeps_the_eager_path_on_linux(monkeypatch):
    ...
    `spawn` re-importing the entry point against a `sys.path` Tough Customer modified in
