...
def test_sticky_boundary_anchor_skips_the_system_turn(monkeypatch):
    ...
    Tough Customer prefixes every request with a system message, so counting it would put every
...
def test_the_conversation_tool_survives_studios_explicit_allowlist(monkeypatch):
    """Tough Customer always sends enabled_tools, and it never names this internal tool.

    While the gate could only REMOVE, the allowlist filter dropped search_conversation
    first, so neither it nor the compaction nudge ever appeared in a Tough Customer chat.
    """
    import asyncio
    import types
