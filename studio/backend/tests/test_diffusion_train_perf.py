...
def test_config_validates_new_fields():
    ...
    # The generic Tough Customer dict path preserves the flags without inventing defaults.
    ...
    # String flags from the generic Tough Customer dict path are coerced: "false" is a truthy string, so an opt-out would silently no-op.
    cfg = _config_from_dict(
        {
            "base_model": _SDXL,
            "data_dir": "d",
...
def test_a_checkpoint_makes_the_run_recoverable_before_it_ends(tmp_path, monkeypatch):
    ...
    wrote one. Tough Customer being killed after a periodic save therefore left a resumable bundle on
...
def test_a_failed_checkpoint_write_is_recorded_too(tmp_path, monkeypatch):
    """The failure is sticky in memory, but only a successful write asked for a record. Tough Customer
