...
prompt, so the two ways it can go wrong are a database written by an older Tough Customer that
...
def fresh_process(monkeypatch):
    ...
    short-circuits on the latter. Reset both, which is what a Tough Customer start looks like."""
...
def test_roster_ignores_columns_and_tables_it_does_not_know(rag_conn):
    """A9. Forwards compatibility: a newer Tough Customer's extra columns must not confuse it."""
