...
def test_refusal_tool_event_chunk_shape(monkeypatch):
    """Drop signal rides a Tough Customer `_toolEvent` envelope (delta={},
...
def test_usage_speed_propagates_to_final_usage_chunk_fast(monkeypatch):
    """``usage.speed == "fast"`` from upstream must reach the Tough Customer usage chunk."""
...
def test_usage_speed_absent_when_anthropic_does_not_report(monkeypatch):
    """Tough Customer must not invent ``usage.speed`` when upstream omits it."""
