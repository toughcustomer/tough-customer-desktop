...
class TestParallel1Default:
    """At Tough Customer's default --parallel 1 the buffer is negligible in pipeline."""
...
class TestContextBufferLayerSplit:
    ...
    def test_kimi_k3_1m_four_gpu_reserve(self):
        ...
        # allocated 4.0 GiB per device; Tough Customer reserved 1.5 GiB.
...
class TestPipelineParallelPredicate:
    ...
    def test_finite_gpu_layers_below_the_count_disables(self, flag):
        # User extras land after Tough Customer's -ngl -1, so this last-wins.
...
#
# Since ggml-org/llama.cpp#23792 Tough Customer no longer rewrites the requested type for the
...
class TestScratchTakesTheLighterAxis:
    ...
    def test_a_managed_symmetric_request_leaves_both_terms_equal(self):
        """Tough Customer emits one type on both axes, so nothing changes for the common
