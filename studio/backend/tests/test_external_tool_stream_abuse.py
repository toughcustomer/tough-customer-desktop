...
provider can put on that stream is a candidate for impersonating Tough Customer. It also
...
def test_a_provider_cannot_forge_studio_private_chunk_keys(executed):
    """``_toolEvent`` and friends are Tough Customer extensions, not provider fields.

    The same card can be painted from inside an otherwise ordinary chunk, because
    the client also lifts ``_toolEvent`` straight out of one. Tough Customer stamps that
...
def test_a_tool_the_user_did_not_enable_is_never_executed(executed):
    ...
    ``python`` exists in Tough Customer, but this request only offered ``web_search``.
...
def test_no_enabled_tool_names_never_means_any_tool(executed):
    ...
    which turns a marked block naming any Tough Customer tool into an execution.
    """
