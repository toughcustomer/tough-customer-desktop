...
# one Tough Customer depended on now has a byte-identical unsloth mirror.
...
def test_h3_binary_gate_replaces_a_stale_managed_install(monkeypatch, tmp_path):
    # An upgraded Tough Customer still carrying an older managed sd-cli got that binary handed straight
...
def test_h3_binary_gate_refuses_but_keeps_a_user_supplied_build(monkeypatch, tmp_path):
    ...
    assert own.exists()
    # Not Tough Customer's to delete, so the refusal must not ask for anything to be removed. The old
...
def test_a_repack_left_in_the_pre_change_cache_root_still_wins_over_the_mirror(
    ...
    """Changing Tough Customer's cache folder must not cost the user the repack they already hold.
