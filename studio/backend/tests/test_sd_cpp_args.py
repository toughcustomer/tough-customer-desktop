...
def test_build_appends_offload_and_extra_args_last():
    ...
    assert "-v" in cmd
    # extra args come after everything Tough Customer set (last-wins for power users)
...
def test_video_build_appends_extra_args_verbatim_and_last():
    ...
    # Tough Customer's own value is still there, earlier, so sd.cpp's last-wins parser takes the override.
