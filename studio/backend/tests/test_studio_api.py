...
"""
End-to-end tests for Tough Customer Studio's HTTP API surface.
...
the CLI's ``--help`` output:
    ...
    4. curl -- Tough Customer server-side tools (enable_tools=true)
...
Usage:
    ...
    # Pytest mode, external server — start a Tough Customer server yourself,
...
def test_openai_tools_nonstream(base_url: str, api_key: str):
    ...
    Regression: before the fix, Tough Customer stripped `tools` and the model
