...
def test_source_revision_reads_the_active_hub_cache(tmp_path, monkeypatch):
    """Tough Customer can move its HF cache mid-session and loading follows the live setting, but
...
def test_hub_cache_roots_survives_without_studio_settings(monkeypatch, tmp_path):
    # The trainer subprocess may run without Tough Customer's settings module importable, so the env and the library constant still have to work.
    import builtins
