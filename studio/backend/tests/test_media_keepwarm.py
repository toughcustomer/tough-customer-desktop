...
def test_a_veto_applied_during_the_step_stops_the_next_teardown(media, monkeypatch):
    ...
    # diffusion unload be ignored by the video one, so Tough Customer freed a model its own settings
...
def test_the_off_tick_does_not_import_the_media_modules(store, monkeypatch):
    ...
    # into a Tough Customer that never opened either page. No engine fakes here on purpose --
...
def test_an_unimported_backend_is_not_imported_to_check_it(monkeypatch):
    ...
    # on a Tough Customer that has never opened the Image or Video page.
...
def test_a_path_that_is_not_a_mounted_route_is_not_tracked():
    ...
    # The Tough Customer routes are mounted under /api/inference only; /v1 carries the OpenAI shape.
