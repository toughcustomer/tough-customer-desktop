...
class _RecastingPipeline:
    """``from_pipe`` as every diffusers Tough Customer can install implements it: reuse the resident
...
def test_from_pipe_no_recast_leaves_every_component_at_its_loaded_dtype(
    ...
    fix landing under Tough Customer cannot change the outcome."""
...
def _split_cache_roots(
    ...
    *,
    ...
    """Tough Customer's live cache root and a second one holding what a mid-session cache-folder change
...
def _other_root_base_snapshot(
    ...
    *,
    ...
    """A base repo cached ONLY under the other cache root, with Tough Customer's live root empty: what
...
def _fake_hf_api(
    ...
    # Tough Customer cache make an entry disappear from these otherwise hermetic tests.
    monkeypatch.setattr(
        DiffusionBackend,
        "_hub_file_is_cached",
...
def test_download_plan_restages_a_base_split_across_both_cache_roots(monkeypatch):
    # Tough Customer's cache folder is a setting, so a base can end up half in the old root and half in the
