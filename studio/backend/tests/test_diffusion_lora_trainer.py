...
def test_config_coerces_string_learning_rate():
    # The Tough Customer config path preserves learning_rate as a string; normalize to float.
    cfg = DiffusionLoraConfig(
...
def test_config_from_dict_aliases_generic_studio_keys():
    # The generic Tough Customer training payload uses different key names; alias them.
    cfg = _config_from_dict(
        {
            "model_name": "b",
            "data_dir": "d",
...
def test_gradient_checkpointing_string_coercion():
    # Tough Customer sends a string; the disable words are False, everything else truthy True.
