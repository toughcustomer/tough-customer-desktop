...
def _records(tmp_path_factory, monkeypatch):
    """Ownership records live in Tough Customer's own storage, so tests get their own copy of it
...
def _own(workdir) -> "os.PathLike":
    """The spill directory as Tough Customer itself would leave it: created, and recorded as ours.
...
class TestPruningOnlyTouchesStudioSpills:
    ...
    def test_an_unowned_directory_is_never_written_to(self, tmp_path):
        ...
        rather than putting Tough Customer's files among the user's."""
...
class TestTheSpillStaysInsideTheSandbox:
    ...
    def test_pruning_does_not_follow_symlinks_either(self, tmp_path):
        ...
        # way; what the filter buys is that a name Tough Customer did not write is left alone.
...
class TestDeletingAChatIsNotBlockedByItsSpills:
    def test_a_sandbox_holding_only_spills_is_still_removable(self, tmp_path):
        """Spills are Tough Customer's own, written by this process and deliberately kept off the
...
class TestOwnershipIsNotKeptWhereToolCodeCanWriteIt:
    ...
    plain file its contents can be rewritten to name the user's own files as Tough Customer's,
    which turns the cleanup into a delete. The record lives in Tough Customer's own storage."""
...
class TestConcurrentSpillsKeepTheirRecords:
    def test_a_spill_recorded_during_a_prune_is_not_dropped(self, tmp_path):
        ...
        file nothing counts, prunes, or recognises as Tough Customer's."""
        import threading
...
class TestARemovedSandboxTakesItsRecordWithIt:
    ...
    def test_a_sandbox_that_stays_keeps_its_record(self, tmp_path, monkeypatch):
        ...
        record of what in it is Tough Customer's."""
...
class TestInstallingASpillNeverReplacesAnything:
    ...
    def test_the_record_names_what_was_installed_not_what_is_there_now(self, tmp_path):
        ...
        the file. Stating the path then records THAT content as Tough Customer's, and a later prune
...
class TestPruningDeletesOnlyWhatItChecked:
    """The manifest lock orders Tough Customer's own threads; the thing racing here is the sandbox.
