...
"""No child outlives the Tough Customer that spawned it.
...
def test_update_gate_is_a_noop_on_posix(tmp_path):
    """On Linux/macOS nothing checks for a running Tough Customer before an update."""
    from unsloth_cli import _studio_runtime_gate
...
def test_macos_style_orphans_are_recorded_and_reaped(tmp_path, monkeypatch):
    ...
    try:
        ...
        # Pretend a previous Tough Customer wrote this and died. The identity is what
...
def test_a_second_studio_does_not_erase_the_first_record(tmp_path, monkeypatch):
    """Two Tough Customer instances can share a home; one record per owner keeps both tracked."""
    from utils import process_lifetime as pl
    ...
    try:
        pl.adopt_pid(child.pid)
        # A sibling Tough Customer's record, written under its own pid.
        import json
...
def test_a_live_owner_is_never_reaped(tmp_path, monkeypatch):
    """Two Tough Customer instances at once must not kill each other's children."""
    from utils import process_lifetime as pl
...
def test_a_child_sharing_studios_group_is_never_recorded():
    """Recording that group would make the sweep kill Tough Customer and every sibling."""
    from utils import process_lifetime as pl
...
def test_ctrl_c_is_passed_on_by_the_console_handler():
    ...
    where the BOOL result is then undefined) would leave Tough Customer unstoppable."""
    import run
...
def test_a_live_owner_survives_an_unreadable_identity(tmp_path, monkeypatch):
    """A `ps` that failed for a moment must not cost a running Tough Customer its
    sidecars."""
    import json
    ...
    try:
        ...
        assert pl.reap_recorded_children() == []
        assert child.poll() is None, "a live Tough Customer's child was killed"
    finally:
        _kill(child.pid)
...
def test_a_zombie_owner_does_not_shield_its_children(tmp_path, monkeypatch):
    ...
    assert terminated == [9501], "a zombie Tough Customer kept its orphans alive"
...
def test_the_installer_announces_and_groups_its_validation_server():
    """The other half of the handoff, in the script Tough Customer runs."""
