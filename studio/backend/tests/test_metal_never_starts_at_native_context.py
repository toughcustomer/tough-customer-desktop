...
class TestTheEmittedCommand:
    ...
    Floor and drop are only correct together: extras are appended after Tough Customer's
...
class TestAutoLayersWithTheFitterTurnedOff:
    ...
    Extras land after Tough Customer's own "--fit on" and win, so a pass-through
...
class TestAnInheritedContextEnvironment:
    ...
    So the command line wins wherever Tough Customer emits one. Auto-layers emits none,
