...
"""Hosted tools that survive a turn the Tough Customer loop runs.
...
a Tough Customer tool, and the loop has to forward those names to the provider instead
of withholding the whole hosted surface: the alternative is a lit toggle for a
tool the model is never offered.

Search and code execution are the opposite case. Tough Customer runs those itself once
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # web_search is Tough Customer's own once the loop runs, so it never rides along.
...
def test_images_plus_a_studio_tool_still_reaches_the_provider(monkeypatch, provider_type):
    """The regression in one line: Images plus Code took the Tough Customer loop, and the
...
def test_the_loop_keeps_its_own_search(monkeypatch):
    """Tough Customer's web_search is running locally this turn, so the provider must not
