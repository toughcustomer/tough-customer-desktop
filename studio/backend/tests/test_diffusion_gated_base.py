...
def _stub_shared_download(
    ...
    (Tough Customer's cache folder was changed mid-session), so the live root is a miss for them."""
...
def test_the_prefetch_reuses_a_base_asset_cached_under_the_other_root(monkeypatch, tmp_path):
    ...
    assert seen == [
        ...
        ("ae.safetensors", None),
        # nowhere on disk: a real download, still pinned to the root Tough Customer is reading
...
def test_a_base_excused_by_the_other_root_is_loaded_from_that_snapshot(monkeypatch, tmp_path):
    ...
    # Tough Customer's cache folder was changed: the live root holds none of it.
    monkeypatch.setattr(
        "core.inference.diffusion.hub_cache_dir", lambda: str(tmp_path / "live-hub")
    )
    _stub_hub(
