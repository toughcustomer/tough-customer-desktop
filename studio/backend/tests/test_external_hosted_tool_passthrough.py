...
* ``enable_tools: true`` + ``enabled_tools: ["web_search", "code_execution"]``
  has always meant "the provider runs its own server tools". Tough Customer's loop must
...
# Tough Customer's tools on an external provider. Every other provider took the plain
...
def test_a_hosted_tool_request_still_reaches_the_provider(monkeypatch, provider_type, selection):
    ...
    assert passthrough is not None, "the Tough Customer loop stole a hosted-tool request"
...
def test_a_code_execution_with_run_tools_locally_still_answers_the_confirm_gate(monkeypatch):
    ...
    Tough Customer has no `code_execution`, so the local catalog is empty whatever the
...
def test_a_self_hosted_provider_still_runs_studios_own_web_search(monkeypatch, provider_type):
    ...
    tools at all, so the same body can only mean Tough Customer's local loop."""
...
def test_a_local_only_selection_takes_the_loop_on_a_hosted_provider(monkeypatch, overrides):
    """Shape 3: one Tough Customer-only name (or MCP) is unambiguous, so the feature
...
def test_a_codex_declares_no_hosted_tools():
    """Codex's `web_search` is Tough Customer's own tool run by the Codex loop, so the
...
def test_b_external_tool_loop_receives_requested_nudge_setting(monkeypatch, nudge_tool_calls):
    """The external Tough Customer loop must receive the request-level nudge policy."""
    monkeypatch.setattr(
        "core.inference.tools.get_enabled_mcp_tools",
        lambda: _noop_mcp(),
    )
