...
def test_the_encoder_config_is_read_from_the_pinned_cache_not_the_default_one(monkeypatch):
    """Tough Customer runs on a configured cache root. An AutoConfig call that ignores it resolves against
    huggingface_hub's import-time default, which re-downloads into a root Tough Customer does not read and
