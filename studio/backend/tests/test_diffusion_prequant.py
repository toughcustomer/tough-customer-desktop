...
def test_the_allowlist_names_every_constructor_the_hosted_checkpoints_use(
    ...
    """The exact set read out of the pickles Tough Customer actually resolves.
...
def test_an_old_torch_registers_nothing_at_all(monkeypatch):
    ...
    list -- so a tuple left there breaks every OTHER weights_only load in Tough Customer too. Hence:
...
def test_an_uncached_checkpoint_downloads_into_the_live_root(monkeypatch):
    # The other half: a real fetch must land where Tough Customer is reading, not under the stale constant.
    asked: list = []
    source = PrequantSource(
...
def test_the_config_follows_the_checkpoint_into_the_other_cache_root(monkeypatch, tmp_path):
    """``_resolve_checkpoint_path`` can answer from huggingface_hub's import-time root while Tough Customer
