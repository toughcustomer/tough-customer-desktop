...
``UnicodeDecodeError`` mid-load. Tough Customer's files are UTF-8, so say so.
"""
...
def test_a_corrupt_pid_file_does_not_abort_shutdown(tmp_path: Path, monkeypatch) -> None:
    ...
    # _legacy_heir scans the real Tough Customer root, so without this the last assertion asks whether
