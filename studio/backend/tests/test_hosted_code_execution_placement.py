...
"""Which side runs the code when a turn mixes the Code pill with a Tough Customer tool.

``code_execution`` runs in the provider's sandbox; ``python`` / ``terminal`` run
on the machine Tough Customer is installed on. They are two trust boundaries, not two
spellings of one feature, so the request says which one it wants by name and
this server forwards accordingly. Tough Customer has no implementation of
...
The one case that IS a substitution is a request naming both, which no Tough Customer
...
def test_hosted_code_execution_rides_along_with_a_studio_tool(provider_type):
    """RAG or MCP selects the Tough Customer loop; the Code pill must still reach the
...
def test_web_search_is_still_never_forwarded():
    """Tough Customer's catalog does contain web_search, so that one really is replaced."""
