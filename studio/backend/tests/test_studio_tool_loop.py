...
"""The provider-agnostic Tough Customer tool loop.
...
def test_a_conversation_search_here_gets_the_active_branch(executed):
    ...
    # roughly 4K tokens to a prompt this loop replays. Tough Customer cannot measure an external
...
def test_a_skipped_duplicate_closes_the_card_the_provider_already_painted(executed):
    ...
    assert ends[1]["result"].startswith("Tough Customer did not run this call")
