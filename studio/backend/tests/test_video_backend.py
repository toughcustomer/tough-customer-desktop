    ...
    class _FakeLTX2Pipeline:
        ...
        def load_config(
            ...
            _FakeLTX2Pipeline.last_config_kwargs = {
                ...
                # Pinned to Tough Customer's LIVE root: unset, this resolves through huggingface_hub's
...
def _plan_api(monkeypatch, repos):
    ...
    # These tests describe their cache state explicitly; never let a developer's real Tough Customer
...
def test_h3_native_reused_cpu_binary_still_commits_to_cpu(monkeypatch, tmp_path):
    ...
    skipped, native_device stayed "cuda", and Tough Customer kept the VIDEO claim and applied GPU offload
...
def test_h3_modular_load_pins_the_component_loads_to_the_studio_cache(fake_runtime):
    ...
    cache folder Tough Customer currently points at, and the two really can differ (it is a live setting).
    """
...
def test_every_video_fetch_resolves_both_cache_roots():
    """The plan probe accepts a file cached under EITHER root (Tough Customer's cache folder is a
...
def test_unified_memory_refuses_an_oversized_video_load(fake_runtime, monkeypatch):
    ...
    allocator's limit, so without this refusal the OS kills Tough Customer with no Python exception.
...
def test_h3_reference_video_trim_tolerates_a_container_longer_than_its_video():
    ...
    longer than it can show, and a browser hands Tough Customer that duration. The last frame is held
...
def test_h3_vae_trim_keeps_the_encoder_for_the_workflows_that_encode():
    """The encoder drop is gated on t2va, and neither workflow Tough Customer loads is text-only.
