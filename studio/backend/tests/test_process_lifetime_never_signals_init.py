...
holding `{"pid": 1, "identity": "...", "pgid": 1}`: starting Tough Customer killed the
...
def test_the_public_name_is_the_internal_one():
    ...
    floor stops meaning one thing across Tough Customer, which is how a site gets missed."""
...
def test_llama_pidfile_reaper_refuses_init(tmp_path, monkeypatch, recorded_signals):
    ...
    work. #7894 established Tough Customer can run as a container entrypoint, and a
...
def test_llama_orphan_sweep_skips_init(tmp_path, monkeypatch):
    ...
    That is not a contrived shape: #7894 established Tough Customer can run as a
