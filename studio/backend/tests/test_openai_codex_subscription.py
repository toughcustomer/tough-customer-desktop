...
def test_chat_drops_a_catalog_another_worker_rebound(monkeypatch):
    ...
    Tough Customer serializes token refreshes across workers on purpose, so this process can hold
...
def test_the_model_route_reports_a_dead_connection(monkeypatch):
    ...
    # Not a 401: authFetch would read that as an expired Tough Customer session, refresh it and
