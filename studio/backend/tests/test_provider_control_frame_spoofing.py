...
"""A provider endpoint must not be able to speak Tough Customer's UI control protocol.
...
def test_a_forged_card_never_survives_the_plain_relay(monkeypatch):
    ...
    harmless, sourced ``local``, on a request where Tough Customer ran no tools at all.
    """
    forged = {
        "type": "tool_end",
        "tool_name": "python",
