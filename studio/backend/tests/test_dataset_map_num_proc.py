...
``None``, not ``1``, is the disable sentinel: on ``datasets`` >= 4.1 (Tough Customer pins
...
def test_num_proc_one_is_not_a_disable_sentinel():
    ...
    ``datasets`` 3.x runs ``num_proc=1`` in-process; 4.x (Tough Customer pins 4.3.0)
...
def test_the_config_value_is_still_in_process_after_the_layer_reads_it(monkeypatch):
    """End to end: what Tough Customer stores, read back the way the SFT config layer reads it."""
    policy = _policy_or_skip()
    _patch_device(monkeypatch, hw.DeviceType.CPU)
...
def test_the_override_is_not_capped_by_the_studio_heuristics(monkeypatch):
    """Uncapped by contract, including by the multi-GPU cap Tough Customer adds after."""
    policy = _policy_or_skip()
    policy.reset_warning_state()
...
def test_an_older_zoo_falls_back_to_the_unsloth_copy(monkeypatch):
    """unsloth.dataset_num_proc is the same policy; Tough Customer should use it too.
...
def test_no_policy_anywhere_keeps_the_previous_behaviour(monkeypatch):
    """Neither module importable: the pre-policy Tough Customer count, not a crash."""
    import builtins
