...
def test_repo_default_and_override(monkeypatch):
    ...
    # Default is the Tough Customer mirror; the env override can point back to leejet upstream.
...
def test_install_downloads_verifies_extracts(tmp_path, monkeypatch):
    ...
    # The ownership marker lets the uninstaller delete a Tough Customer-installed sd.cpp while keeping a user's own checkout.
...
def test_install_into_nonempty_unowned_dir_is_refused(tmp_path, monkeypatch):
    # A pre-existing, non-empty directory Tough Customer did not create must not be extracted into; install() refuses up front.
    zb = _zip_with_sd_cli()
    _stub_release(monkeypatch, zip_bytes = zb, digest = "sha256:" + hashlib.sha256(zb).hexdigest())
    ...
    with pytest.raises(RuntimeError, match = "not a Tough Customer-managed directory"):
...
def test_partial_install_failure_is_reclaimed_on_retry(tmp_path, monkeypatch):
    ...
    # recognises the debris as ours and re-extracts instead of tripping the "not a Tough Customer-managed directory" refusal.
    zb = _zip_with_sd_cli()
    _stub_release(monkeypatch, zip_bytes = zb, digest = "sha256:" + hashlib.sha256(zb).hexdigest())
...
def test_safe_extractall_rejects_a_symlink_onto_a_reserved_installer_path(tmp_path):
    # The marker exists before extraction on any root Tough Customer owns, so a link to it resolves
...
def test_find_sd_cpp_binary_honors_studio_home(tmp_path, monkeypatch):
    # A binary installed under a custom Tough Customer root must be discovered without also setting UNSLOTH_SD_CPP_PATH.
...
def test_managed_root_is_under_the_studio_home_like_every_other_component(tmp_path, monkeypatch):
    """The sd.cpp tree installs *under* the Tough Customer home, not beside it.
...
def test_the_legacy_sibling_tree_is_adopted_only_when_it_carries_the_marker(tmp_path, monkeypatch):
    ...
    likely to be someone's clone than a previous Tough Customer install."""
    from core.inference import sd_cpp_engine as eng
...
# ── Tough Customer mirror: default source + the CPU/Apple asset set it publishes ─────
...
def test_unrunnable_binary_in_an_unmarked_root_is_kept_because_install_would_refuse(
    ...
    def _refuse(**_kwargs):
        raise RuntimeError("sd.cpp install target already exists and is not a Tough Customer-managed dir")
...
def _managed_tree(
    ...
    """A Tough Customer-owned install tree, optionally carrying an install record."""
    root = tmp_path / "sd-home" / "stable-diffusion.cpp"
    (root / "sd-bin").mkdir(parents = True)
    (root / ".unsloth-studio-owned").touch()
    monkeypatch.setenv("UNSLOTH_STUDIO_HOME", str(tmp_path / "sd-home" / "studio"))
...
def test_a_legacy_sibling_install_is_read_from_its_own_root(tmp_path, monkeypatch):
    ...
    A tree an older build installed BESIDE the Tough Customer home is still found by the finder, but the
...
def test_an_external_record_update_retires_the_memo(tmp_path, monkeypatch):
    ...
    Tough Customer rewrites that file, the file is the newer answer -- otherwise this process would keep
...
def test_a_cancelled_request_leaves_the_install_wait(tmp_path, monkeypatch):
    ...
    get out of it reads as a hung Tough Customer for up to the whole 900s while nothing has even started.
...
def _owned_tree_holding(tmp_path, rel: str) -> Path:
    """A Tough Customer-owned install dir already carrying a previous bundle's sd-cli at ``rel``."""
...
def test_a_serverless_install_does_not_fall_back_to_the_legacy_server(tmp_path, monkeypatch):
    """The finder also probes the tree beside the Tough Customer home, so when the bundle just installed
...
def test_a_serverless_install_is_not_downloaded_again_on_every_later_load(tmp_path, monkeypatch):
    """Rejecting the legacy server is only half the answer. The tree beside the Tough Customer home keeps
