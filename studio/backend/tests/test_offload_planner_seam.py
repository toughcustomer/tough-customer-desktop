...
def test_a_plan_that_already_fits_emits_no_flags():
    """The planner disagreeing with Tough Customer ("it fits after all") is not licence
...
def test_the_measured_cache_floors_the_planners_own_estimate():
    ...
    cache Tough Customer already sized. Under is the dangerous direction: the deficit
...
def test_the_projector_and_mtp_reserve_reach_the_planner():
    ...
    Leaving them out makes the deficit too small on exactly the loads Tough Customer
...
def test_an_engaged_draft_charges_the_excluded_mtp_blocks():
    ...
    n_layer_all (llama-model.cpp:1449) places them on a GPU first. Tough Customer's own
...
def test_the_seam_hands_the_planner_raw_free_vram_for_the_split():
    ...
    Tough Customer's budget, so the seam has to pass the raw numbers separately. Observed
...
def test_a_pass_through_parallel_that_grows_the_cache_declines_the_plan():
    """Slots are sizing, not placement. Tough Customer's --parallel is emitted first and
