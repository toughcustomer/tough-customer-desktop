...
"""Diffusion LoRA training for Tough Customer Studio (text-to-image, SDXL).

Trains a LoRA adapter on the U-Net of an SDXL pipeline from an image + caption dataset
and exports it as a diffusers-format ``.safetensors`` that the Tough Customer diffusion backend
    ...
    *,
    ...
    try:
        ...
        for opt_step in range(resumed, cfg.train_steps):
            ...
            # max_grad_norm at or below 0 disables clipping (Tough Customer sends 0.0); passing 0.0 to clip_grad_norm_
            ...
            if done % cfg.log_every == 0 or done == cfg.train_steps:
                # ``learning_rate`` (not ``lr``) is the field the Tough Customer training pump reads.
                if device == "cuda":
