...
def _hf_token_in_play(hf_token: Optional[str]) -> bool:
    """Whether the failing Hub call carried ANY credential, not just Tough Customer's own: with token=None
...
def _repo_access_message(repo: str, *, gated: bool) -> str:
    ...
    if gated:
        return (
            ...
            "Tough Customer settings and try again."
        )
    return (
        ...
        "has access in Tough Customer settings and try again."
    )
...
def _assert_base_repo_accessible(
    ...
    *,
    ...
    def _already_downloaded() -> bool:
        """True when ``probe_file`` is on disk under EITHER root (Tough Customer pins its live setting, the
...
class DiffusionBackend:
    ...
    def _run_load(self, **kwargs: Any) -> None:
        ...
        try:
            ...
            # surfaced verbatim and Tough Customer can be shared. Guarded because on this daemon thread anything escaping
    ...
    def _load_dense_quant_pipeline(
        ...
        *,
        ...
        peak. Verified on the Tough Customer stack: scale 0 reproduces the quantized base exactly.
