...
torch imported lazily so this stays importable in a no-torch runtime. Tough Customer's hardware layer
...
def resolve_diffusion_device_target(*, ordinal: Optional[int] = None) -> DiffusionDeviceTarget:
    ...
    Prefers Tough Customer's hardware layer, else probes torch (CUDA -> XPU -> MPS -> CPU). On Apple
    Silicon Tough Customer may report MLX/CPU, but diffusers uses MPS, so those fall through to the MPS
