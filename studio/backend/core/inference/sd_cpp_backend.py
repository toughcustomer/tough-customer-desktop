...
def _tree_reader(
    ...
    cancel that could not get out of this would read as a hung Tough Customer for up to the whole timeout
...
# Max images per img_gen job; larger Tough Customer batches (up to 32) are split into these chunks
_MAX_SERVER_BATCH = 8
...
def _usable_or_discard_managed(binary: str) -> bool:
    ...
    if not is_managed_binary(binary):
        logger.warning(
            "sd.cpp binary %s is not runnable; leaving it alone (not a Tough Customer-owned install we may "
            "replace). Delete its directory to have Tough Customer reinstall the prebuilt.",
            binary,
        )
...
def _h3_replacement_hint(binary: str) -> str:
    ...
    for root in roots:
        ...
        try:
            ...
            continue
        return f", or move {root} aside so Tough Customer can install the pinned prebuilt there"
    return ""
...
def ensure_h3_sd_cpp_binary(
    *, allow_install: bool = True, accelerator: str = "cpu"
    ...
    runnability, so an install that predates H3 (an upgraded Tough Customer still carrying an older managed
...
def _superseded_legacy_server(binary: Optional[str], accelerator: str) -> bool:
    ...
    the Tough Customer home, while the CURRENT managed root holds a completed install for ``accelerator``
...
def _installed_accelerator_of(binary: Optional[str]) -> Optional[str]:
    ...
    # left beside the Tough Customer home, and reading the current root for a binary out of that one reports "unrecorded" on
...
def ensure_sd_server_binary(
    *, allow_install: bool = True, accelerator: str = "cpu"
    ...
    with _install_lock:
        ...
        # The finder also probes the tree an older build left beside the Tough Customer home, so when the bundle just installed
