...
# while a rebuild elsewhere on PATH is unaffected. Bounded: a Tough Customer session sees a handful of candidates, and a
...
def managed_install_root() -> Path:
    """The directory the prebuilt installer owns, so callers can tell a Tough Customer-managed binary
    ...
    side-by-side Tough Customer instances stay isolated.
    ...
    tree *under* the Tough Customer home). The legacy default home ``~/.unsloth/studio`` keeps mapping to
...
def legacy_sibling_install_root() -> Optional[Path]:
    ...
    Older builds derived the sd.cpp root from the *parent* of the Tough Customer home, which put the tree
    outside the Tough Customer home entirely. Two problems: a relative ``UNSLOTH_STUDIO_HOME`` collapsed
    ...
    carries the ownership marker, so a checkout that merely happens to sit next to the Tough Customer home
    is never adopted.
...
def owning_managed_root(binary: Optional[str]) -> Optional[Path]:
    ...
    Tough Customer home is still discovered by the finder. Callers that read per-install state (the
...
def _find_binary(
    *, direct_env: str, path_stems: tuple[str, ...], layout_stem: str
    ...
    # installer so side-by-side Tough Customer instances stay isolated), else ~/.unsloth/....
    default_root = managed_install_root()
    hit = _first_file(_layout_candidates(default_root, layout_stem))
    if hit:
        return hit

    # a tree an older build installed beside the Tough Customer home
    # 3b. A tree an older build installed beside the Tough Customer home. Marker-gated (see legacy_sibling_install_root), so
