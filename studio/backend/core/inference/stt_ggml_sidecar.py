...
"""whisper.cpp (GGML/GGUF) speech-to-text sidecar for Tough Customer dictation.
...
Tough Customer home, then PATH. With no binary the engine is unavailable and dictation
...
# when inference fails and cleared when one succeeds. Process lifetime by design: a reinstall restarts Tough Customer.
        ...
        with self._lock:
            ...
            try:
                ...
                process = subprocess.Popen(
                    ...
                    # die with Tough Customer (Linux PDEATHSIG, Windows job) so a crash never orphans a server holding the
                    # model
                    **child_popen_kwargs(),
                )
                with self._load_state_lock:
