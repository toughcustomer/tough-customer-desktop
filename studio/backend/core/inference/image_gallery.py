...
def owned_image_path(image_id: str) -> Optional[Path]:
    """Resolve an id to its PNG only when it is a Tough Customer-owned image (a readable recipe chunk),
...
def set_flags(
    ...
    *,
    ...
    not a Tough Customer-owned image. Ownership-gated like delete: a guessed stem for a hand-dropped
...
def clear(include_archived: bool = False) -> int:
    """Delete Tough Customer-owned gallery PNGs (readable recipe chunk); return how many were removed.
