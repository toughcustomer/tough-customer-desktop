...
so Tough Customer has one idle mechanism rather than one per backend.
"""
...
_pending = 0
# Subset of _pending that is /p/ preview traffic, so the busy guard can tell a queued Tough Customer request from a queued
# preview.
_preview_pending = 0
...
# Tough Customer: clearing the marker on an image or video generation would leave a still-preview-owned checkpoint looking
# Tough Customer-owned, and the next preview for a different checkpoint would 503 on the slot guard.
...
def other_non_preview_pending_count() -> int:
    ...
    The preview swap guard must count these: a queued Tough Customer request would otherwise start
...
def _claim_non_preview_slot() -> None:
    """A non-preview request that ran against the local model (2xx) adopts it for Tough Customer,
    so clear preview ownership -- a later preview for another checkpoint then 503s instead
    of swapping the model out from under an active Tough Customer conversation. Claiming on success
...
# a Tough Customer swap holds the lifecycle gate while draining active requests, and the queued preview needs that same gate
...
# adopt a preview-owned model for Tough Customer; the claim skips a flagged response.
...
def mark_response_failed(scope) -> None:
    ...
    treat it as a successful non-preview completion and claim the slot for Tough Customer. Safe to
...
class LlamaKeepWarmMiddleware:
    ...
    async def __call__(self, scope, receive, send):
        ...
        ended = {"done": False}
        status = {"code": None}
        # set once the terminal body frame is sent: only a cleanly completed response adopts the model for Tough Customer.
        # Set once the terminal body frame (more_body False) is sent: only a response that completed cleanly adopts the
        # model for Tough Customer. A client disconnect after the 200 headers raises before that frame (an OSError that
        ...
        def _finish() -> None:
            ...
            if not chat_tracked:
                return
            # A non-preview 2xx that completed cleanly ran against the local model and adopts it for Tough Customer, so clear
            ...
            # Tough Customer traffic and a still-preview-owned slot, swaps in, and this delayed claim then clears the wrong
