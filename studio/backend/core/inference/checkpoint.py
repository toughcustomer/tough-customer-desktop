...
# caller's policy. Only a block Tough Customer itself rendered carries this header, so only that one is claimed.
_BLOCK = re.compile(
...
def _without_block(messages: list[dict]) -> list[dict]:
    """``messages`` with any block Tough Customer rendered removed from the system turn.
