...
def _helpers() -> Optional[dict]:
    ...
    and that is set by ``unsloth`` itself. The Tough Customer server imports ``unsloth`` at boot so this
    ...
    False). So on failure, import ``unsloth`` and retry once, which is also the import order Tough Customer
    documents.
