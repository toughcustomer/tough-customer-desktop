...
def _prune_to_cap() -> int:
    ...
    Best-effort, so a save never fails on housekeeping. Only Tough Customer-owned pairs are eligible,
...
def owned_audio_path(audio_id: str) -> Optional[Path]:
    """Resolve an id to its WAV only for a Tough Customer-owned clip (readable sidecar).
...
def set_flags(audio_id: str, *, archived: Optional[bool] = None) -> Optional[dict[str, Any]]:
    """Archive or restore one owned clip; None when the id is not a Tough Customer-owned clip."""
...
def clear(include_archived: bool = False) -> int:
    """Delete every Tough Customer-owned pair (readable sidecar); return the count removed. Foreign and
