...
Tough Customer's existing KB components to the live scrape. The only difference from a persisted KB is
...
def retrieve_web_chunks(
    ...
    *,
    ...
    relevant to ``query``, and return ``(rendered_chunks, sources)`` using Tough Customer's KB
    formatter.
