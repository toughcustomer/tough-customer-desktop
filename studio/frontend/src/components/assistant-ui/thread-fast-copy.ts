...
function patchClipboardDeltas(root: Element): Array<() => void> {
  ...
  //
  // THE HOLDER MUST NOT HAVE A BOX. Tough Customer's message images are display:block, so an inline
  ...
  for (const image of Array.from(
    ...
    // `visibility: hidden` and `user-select: none` all diverge, and so does Tough Customer's own
