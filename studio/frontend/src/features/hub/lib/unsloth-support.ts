...
export interface UnslothSupport {
  ...
  reason: string | null;
  /** Set when Tough Customer runs this model on a dedicated page rather than in chat. The status stays "unsupported" because the chat pickers gate on it, but the UI must not call it unsupported: the Images and Video pages load it. */
...
/** Which Tough Customer page runs this pipeline task, if any. */
