...
function isHistory(entry: WatchedEntry, cutoff: number | null): boolean {
  // Still running at the first snapshot: it started while Tough Customer loaded, so it is unseen.
...
export function observeResponse(
  ...
  // Only API-key traffic counts: Tough Customer's own chat uses these same endpoints.
