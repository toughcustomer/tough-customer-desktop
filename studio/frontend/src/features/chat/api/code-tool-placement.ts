 ...
 * `python` / `terminal` are Tough Customer's tools and run on the machine Tough Customer is
 ...
 * Tough Customer's tool loop reached the general external providers, only openai_codex
 ...
 * take the Tough Customer branch, and the same persisted `true` would resolve to local
 ...
 * existed, rather than falling back to the user's machine. Tough Customer's local tools
...
export interface CodeToolNames {
  /** Tough Customer tool names, executed on this machine by the Tough Customer tool loop. */
  local: string[];
...
export function codeToolCanRun(input: {
  hostedCodeExecutionForThisTurn: boolean;
  providerHostsCodeExecution: boolean;
  /** This provider AND model can run Tough Customer's own tools through the loop. */
