...
const MMPROJ_FALLBACK_MESSAGES: Record<MmprojFallbackReason, string> = {
  ...
  cpu_offload:
    "Tough Customer is running the vision projector in system memory rather than on the GPU. Image input remains available, but image processing may be slower.",
  projector_incompatible:
    "The vision projector is incompatible with the installed llama.cpp build, so Tough Customer reloaded this model in text-only mode. Update Tough Customer, then reload the model to restore image input.",
  projector_startup_failure:
    "The vision projector could not start on the GPU or CPU, so Tough Customer reloaded this model in text-only mode. Free memory or check the GPU logs, then reload the model to restore image input.",
};
