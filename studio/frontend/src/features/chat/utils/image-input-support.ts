...
function textOnlyMmprojUnavailableReason(
  ...
  return `${label}'s vision projector failed to start, so Tough Customer reloaded it in text-only mode. Free memory or update Tough Customer, then reload the model before attaching images.`;
}
