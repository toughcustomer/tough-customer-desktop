  ...
  if (kind === WORKING_DIRECTORY_UNAVAILABLE) {
    const cause =
      typeof navigator !== "undefined" && /Win/i.test(navigator.platform ?? "")
        ...
        : "";
    return `Tough Customer cannot reach your user folder, so it has nowhere to run from.${cause} Reconnect and try again.`;
  }
  // Also not an install problem, and not the folder either: one of Tough Customer's own
  // path settings names somewhere unresolvable, so the value is the fix.
  if (kind === PATH_SETTING_UNRESOLVABLE) {
    const which = setting ? `${setting} points` : "One of Tough Customer's folder settings points";
    return `${which} somewhere that cannot be resolved, so Tough Customer has nowhere safe to run from. Set it to a full path, such as D:\\unsloth-cache, and try again.`;
  }
  if (disposition === "owned_stale") {
    return "Desktop-owned Tough Customer backend is too old for this desktop app. use the Update action in Settings, then restart Tough Customer.";
  }
  return "Managed Tough Customer install is too old. use the Update action in Settings.";
}
